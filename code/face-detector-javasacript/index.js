...
Promise.all([TINY_FACE_DETECTOR, FACE_LANDMARK, FACE_RECOGNITION, FACE_EXPRESSION, FACE_AGE_GENDER]).then(startDetect);
  ...
  setInterval(async () => {
    ...
    resizedDetections.forEach(({ age, gender, detection }) => {
      const drawbox = new faceapi.draw.DrawBox(detection.box, {
        label: [`umur : ${faceapi.round(age, 0)}`, `Sex : ${gender === 'male' ? 'Laki-Laki' : 'Perempuan'}`],
      });
      drawbox.draw(canvas);
    });
  }, 100);
